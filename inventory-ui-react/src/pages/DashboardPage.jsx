...
const DashboardPage = () => {
  ...
  //variabble to store and set transaction data formated for chart display
  const [transactionData, setTransactionData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log("fetchData from: ", selectedMonth, selectedYear, selectedData);
  ...
  const transformTransactionData = (transactions, month, year) => {
    const dailyData = {};
    //get number of dayas in the selected month year
    ...
    transactions.forEach((transaction) => {
      const transactionDate = new Date(transaction.createdAt);  //"2025-11-17T17:18:24.682842"
      const transactionMonth = transactionDate.getMonth() + 1;
      const transactionYear = transactionDate.getFullYear();
      //console.log("transactionDate: ", transactionDate, transactionMonth, transactionYear);
